...
def level2_construct_file_info(level1_files: list[File], pipeline_config: dict, reference_time=None) -> list[File]:
    files = [File(
                ...
                date_beg=min([f.date_beg for f in level1_files]),
                date_obs=average_datetime([f.date_obs for f in level1_files]),
                date_end=max([f.date_end for f in level1_files]),
    ...
    for obs in unique_obses:
        ...
        files.append(File(
                ...
                file_version=pipeline_config["file_version"],
                software_version=__version__,
                date_beg=min([f.date_beg for f in input_files]),
                date_obs=files[0].date_obs,
                date_end=max([f.date_end for f in input_files]),
