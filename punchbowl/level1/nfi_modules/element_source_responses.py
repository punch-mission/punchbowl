...
#@processify
def element_source_responses(source, detector, transform, nbuf = 10**7, dtype='float32', silent=False):
    ...
    for i in range(0,source.nadr): # Loop over each source address
        ...
        # Print the status:
        if(silent==False and ((i+1) % int(shape[1]/20)) == 0): print(100*i/(shape[1]-1),'% done after',time.time()-t0,'seconds')
