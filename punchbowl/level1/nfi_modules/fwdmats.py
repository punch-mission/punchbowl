...
def generate_nfi_fwdmats(datsiz, xoffs, yoffs, crots, bin_fac=4, smooth_rad=0.05, radial_size=175.4, elon_abs=130, cx=1009, cy=1029, nstray=None):
	...
	amat_inst = esr(source_inst, detector0, coord_transform)
	#print('Using diagonal forward matrix for per-pixel instrument component')
	#amat_inst = eye(np.prod(dims))
	detectors, amats_sky = [], []
	for i in range(0,nframe):
		print('Detector '+str(i)+'; crota='+str(crots[i]))
		detectors.append(get_detector(dims, center=[xoffs[i], yoffs[i]], crota=crots[i]))#0.1*np.pi)#, rotarr[1]))
		amats_sky.append(esr(source_sky, detectors[i], coord_transform))

	im_size = dims[0]
	# Note we do not include the endpoint of the interval
	# since that would put duplicate kernels at 0 and 2*pi...
	if(nstray is None): nstray = im_size
	kernel_angles = 2*np.pi*np.arange(nstray)/nstray
	kernels = gen_kernels(kernel_angles, radial_size=radial_size, elon_abs=elon_abs, image_size=im_size, cx=cx/bin_fac, cy=cy/bin_fac)
	k2 = np.array(kernels)
	for i in range(0,len(k2)): k2[i] = k2[i].T
...
def generate_stray_fwdmats(datsiz, xoffs, yoffs, crots, bin_fac=4, smooth_rad=0.05):
	nframe, nx0, ny0 = datsiz
	dims = np.round(np.array([nx0,ny0])/bin_fac).astype(np.int32)
	#dx = hdr['CDELT1']*bin_fac; dy = hdr['CDELT2']*bin_fac
	print(dims)
	...
	#xoffs = np.array([hdr['CRVAL1'] for hdr in hdrs])/dx
	#yoffs = np.array([hdr['CRVAL2'] for hdr in hdrs])/dy
	#crots = np.array([hdr['CROTA'] for hdr in hdrs])*np.pi/180
...
#import importlib
#import sharpesst.solver
#importlib.reload(sharpesst.solver)
...
def get_sky_source(dims, crota=0.0, center=np.array([0,0]), scale=[1.0,1.0], src_subgrid_fac=2):
	...
	src_coords = coord_grid(dims, origin, fwdtransform, src_frame)
	return source_grid(src_coords, None, bin_function, nsubgrid=src_subgrid_fac)
...
def get_detector(dims, crota=0.0, center=np.array([0,0]), scale=[1.0, 1.0], det_subgrid_fac=3):
	...
	#origin = fwdtransform.dot(-0.5*dims)+center
	origin = fwdtransform.dot(-0.5*dims+center)
	det_coords = coord_grid(dims, origin, fwdtransform, det_frame)
...
def csc_resize(csc, rsiz, csiz, r0, c0):
	...
	cinds = csc.indptr
	if(c0 > 0): cinds = np.hstack([np.zeros(c0,dtype=np.int64),cinds])
